...
import com.example.liulu.accumulations.adapter.GoodsAdapter;
import com.example.liulu.accumulations.common.BaseActivity;
import com.example.liulu.accumulations.model.GoodsModel;

import java.util.ArrayList;

import butterknife.Bind;

public class ShoppingcarBActivity extends BaseActivity {

    @Bind(R.id.iv_shopping_cart)
    ImageView mShoppingCartIv;
    @Bind(R.id.tv_count)
    TextView mShoppingCartCountTv;
    @Bind(R.id.fly_shopping_cart)
    FrameLayout fly_shopping_cart;
    @Bind(R.id.shopping_cart)
    ListView mShoppingCartLv;
    @Bind(R.id.rly_shopping_cart)
    RelativeLayout rlyShoppingCart;
    ...
    private void addGoodsToCart(ImageView goodsImg) {
        ...
        rlyShoppingCart.addView(goods, params);

        // 得到父布局的起始点坐标（用于辅助计算动画开始/结束时的点的坐标）
        int[] parentLocation = new int[2];
        rlyShoppingCart.getLocationInWindow(parentLocation);
        ...
        valueAnimator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                // 把执行动画的商品图片从父布局中移除
                rlyShoppingCart.removeView(goods);
            }
    ...
    public int getLayout() {
        return R.layout.activity_shoppingcar_b;
    }
